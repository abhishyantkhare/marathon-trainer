...
4. Triggers a frontend preview redeploy so the new env var is applied

Required environment variables:
- COFOUNDER_VERCEL_TOKEN: Your Vercel API token
...
def get_vercel_token() -> str:
    ...
    token = os.environ.get("COFOUNDER_VERCEL_TOKEN")
    if not token:
        print("Error: COFOUNDER_VERCEL_TOKEN environment variable is not set")
        ...
        for key in sorted(os.environ.keys()):
            if "TOKEN" in key or "VERCEL" in key or "BRANCH" in key:
                ...
                print(f"  {key}={value}")
        sys.exit(1)
    print(f"COFOUNDER_VERCEL_TOKEN found (length: {len(token)})")
    return token
...
def get_latest_preview_deployment_id_for_branch(
    token: str,
    project_id: str,
    branch: str,
) -> str | None:
    """Get the most recent preview deployment id for a branch on a project."""
    print(f"Fetching latest preview deployment id for branch '{branch}'...")
    ...
    if response.status_code != 200:
        print(f"Warning: Failed to fetch frontend deployments: {response.status_code}")
        print(f"Response: {response.text}")
        return None
    ...
    if not deployments:
        print(
            "Warning: No frontend preview deployments found for this branch. "
            "Skipping redeploy."
        )
        return None
    ...
    for deployment in deployments:
        ...
        if deployment_id:
            print(f"Found frontend deployment id: {deployment_id}")
            return deployment_id

    print(
        "Warning: Could not find a frontend preview deployment id for this branch. "
        "Skipping redeploy."
    )
    return None


def redeploy_preview_deployment(token: str, deployment_id: str) -> str | None:
    """Redeploy an existing deployment and return the new deployment URL if available."""
    print(f"Triggering frontend redeploy from deployment id '{deployment_id}'...")
    ...
    if response.status_code not in [200, 201]:
        print(f"Warning: Failed to trigger redeploy: {response.status_code}")
        print(f"Response: {response.text}")
        return None

    new_deployment = response.json()
    url = new_deployment.get("url")
    if url and not url.startswith("https://"):
        url = f"https://{url}"

    print("Triggered frontend redeploy successfully")
    return url
...
def main():
    ...
    print()
    print("Step 8: Redeploying frontend preview deployment to pick up env var...")
    frontend_deployment_id = get_latest_preview_deployment_id_for_branch(
        token=token,
        project_id=frontend_project_id,
        branch=branch,
    )
    new_frontend_url = None
    if frontend_deployment_id:
        new_frontend_url = redeploy_preview_deployment(token, frontend_deployment_id)
    print()
    print("=" * 60)
    print("Successfully set preview environment variable and triggered redeploy!")
    print(f"  Project: {frontend_project_name}")
    print(f"  {env_var_name}={deployment_url}")
    if new_frontend_url:
        print(f"  New frontend deployment URL: {new_frontend_url}")
    print("=" * 60)
